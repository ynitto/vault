---
title: "Obsidian でモーダルから入力できるプラグイン Modal Opener｜MaybeFix"
type: "topic"
tags:
  - "obsidian"
  - "code-review"
  - "resource-ingest"
created: "2026-05-02"
updated: "2026-05-02"
sources:
  - "/Users/nitto/Library/Mobile Documents/iCloud~md~obsidian/Documents/MyVault/60_Resources/Obsidian でモーダルから入力できるプラグイン Modal Opener｜MaybeFix.md"
summary: "Obsidian プラグイン「Modal Opener」"
---

# Obsidian でモーダルから入力できるプラグイン Modal Opener｜MaybeFix

## 概要

Obsidian プラグイン「Modal Opener」

*発行: 2024-11-17 / [[obsidian-code-review-obsidian-modal-opener-c13dc3]]*

## 主要なトピック

- [[obsidian]]
- [[code-review]]

## 詳細

- 概要
- Obsidianのリンクをモーダルウィンドウで開閉できるプラグイン。
- 編集状態（Alt+左クリック）またはプレビュー状態（Alt+右クリック）で開くことが可能。
- 新規リンク（未作成ページ）はモーダルウィンドウで開けない。
- ホバープレビュー・Hover Editorとの比較
- **Modal Opener**: 入力に集中できるシンプルなインターフェース。サイズ変更不可。
- **ホバープレビュー**: Ctrl+マウスオーバーで閲覧可能。最近のアップデートで編集も可能になった。
- **Hover Editor**: ホバープレビューの編集不可を補うために開発。ウィンドウ化やサイズ変更が可能。多機能。
- 特徴

*発行: 2024-11-17 / [[obsidian-code-review-obsidian-modal-opener-c13dc3]]*

## 関連テーマ

- [[obsidian]]
- [[code-review]]

## 出典

- `/Users/nitto/Library/Mobile Documents/iCloud~md~obsidian/Documents/MyVault/60_Resources/Obsidian でモーダルから入力できるプラグイン Modal Opener｜MaybeFix.md`
- https://note.com/maybefix/n/n0adffb4ebfe3
