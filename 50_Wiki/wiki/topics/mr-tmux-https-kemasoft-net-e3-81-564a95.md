...
- 実装の手順
- **ディレクトリ作成**: ログ保存用に `~/50_Wiki/wiki/topicstmux` を作成します。
- **フィルター用スクリプト**: `awk` で日時を付与し、`sed` で制御コードを削除する `tslog` スクリプトを作成し、`/usr/local/bin` 等に配置します。
- 設定方法
- `~/50_Wiki/wiki/topicstmux.conf` に以下のキーバインドを追加します。
- **ログ開始**: `bind-key H pipe-pane 'tslog >\\"$HOME/50_Wiki/wiki/topicstmux/%Y%m%d-%H%M%S-#P.log\\"'`
